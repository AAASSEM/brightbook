...
from app.models.models import Child, Assessment, Activity, ActivityProgress, Progress
...
def fix_child_activities(child_id: int):
    with Session(engine) as session:
        ...
        assessment = session.exec(select(Assessment).where(Assessment.Child_ID == child_id, Assessment.is_initial == True)).first()
        if not assessment:
            print("No assessment found for this child.")
            return

        # Delete existing activities and progress
        activities = session.exec(select(Activity).where(Activity.Child_ID == child_id)).all()
        for act in activities:
            aps = session.exec(select(ActivityProgress).where(ActivityProgress.activity_id == act.Activity_ID)).all()
            for ap in aps:
                session.delete(ap)
            session.delete(act)
        session.commit()

        print(f"Deleted old activities. Generating new activities for {child.name}...")
        
        # Enforce dyslexia level 2 based on assessment
        level = 2
        ...
        for activity_data in activities_data:
            try:
                ...
                # Default to a safe supported type if AI still hallucinates
                supported_types = ['meet_letter', 'hear_sound', 'trace_write', 'mini_quest', 'sound_blender', 'word_builder', 'read_match', 'say_yourself']
                if activity_type_str not in supported_types:
                    activity_type_str = 'meet_letter'
                ...
                is_arabic = (child.native_language or "English").lower() in ["arabic", "ar"]
                expected_group = f"arabic_group_{level}" if is_arabic else f"group_{level}"
